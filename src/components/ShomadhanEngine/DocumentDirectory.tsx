...
export const DocumentDirectory: React.FC<DocumentDirectoryProps> = ({
  ...
  const documentCategories = [
    {
      key: 'lost_nid',
      icon: <CreditCard className="w-5 h-5 text-emerald-700 dark:text-emerald-400" />,
    ...
    },
    {
      ...
      icon: <FileCheck className="w-5 h-5 text-cyan-700 dark:text-cyan-400" />,
    ...
    {
      key: 'university_id',
      icon: <GraduationCap className="w-5 h-5 text-amber-700 dark:text-amber-400" />,
    ...
    {
      key: 'bank_dispute',
      icon: <ShieldAlert className="w-5 h-5 text-rose-700 dark:text-rose-400" />,
    ...
    {
      key: 'driving_license',
      icon: <Award className="w-5 h-5 text-indigo-700 dark:text-indigo-400" />,
    ...
    },
    {
      ...
      icon: <FileText className="w-5 h-5 text-teal-700 dark:text-teal-400" />,
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between px-1">
        <div>
          <h3 className="font-extrabold text-lg text-[#0F172A] dark:text-slate-100 flex items-center space-x-2">
            <span>Bangladeshi Official Document & Card Directory</span>
          </h3>
          <p className="text-xs text-[#334155] dark:text-slate-400 font-bold mt-0.5">
          ...
          return (
            <div
              ...
              className={`p-4 rounded-2xl border cursor-pointer transition-all duration-200 space-y-3 ${
                isActive
                  ? 'bg-[#DCECC7] dark:bg-emerald-950/30 border-emerald-600 dark:border-emerald-500 shadow-lg ring-2 ring-emerald-500/40'
                  : 'bg-[#EBE3A7] dark:bg-[#131924] border-[#D9D092] dark:border-slate-800/80 hover:border-emerald-500/40 shadow-sm hover:shadow-md'
              }`}
            >
              <div className="flex items-center justify-between">
                <div className="p-2 rounded-xl bg-[#DCD396] dark:bg-slate-900/80 border border-[#C5BC7F] dark:border-slate-800">
                  {cat.icon}
                </div>
                <span className="text-[10px] font-extrabold px-2.5 py-0.5 rounded-full bg-[#DCD396] dark:bg-slate-800 text-[#0F172A] dark:text-slate-200 border border-[#C5BC7F] dark:border-slate-700 shadow-sm">
                  {cat.badge}
                </span>
              </div>
              ...
              <div>
                ...
                <p className="text-xs text-[#047857] dark:text-emerald-400 font-extrabold mt-0.5">
                  {language === 'bn' ? cat.feeBn : cat.feeEn}
                </p>
              </div>

              <p className="text-xs text-[#1E293B] dark:text-slate-400 font-medium line-clamp-2 leading-relaxed">
                {language === 'bn' ? cat.descBn : cat.descEn}
              </p>

              <div className="flex items-center justify-between text-xs pt-2 border-t border-[#D9D092] dark:border-slate-800/60">
                <button
                  type="button"
                  onClick={(e) => {
                    e.stopPropagation();
                  ...
                  className="text-[#047857] dark:text-emerald-400 font-extrabold hover:opacity-90 flex items-center space-x-1 py-1.5 px-3 rounded-lg bg-emerald-500/20 dark:bg-emerald-500/10 border border-emerald-600/40 dark:border-emerald-500/30 transition-all shadow-sm"
                >
                ...
                <a
                  ...
                  className="text-[10px] text-[#334155] dark:text-slate-400 hover:text-cyan-700 dark:hover:text-cyan-400 font-extrabold hover:underline"
                >
                  Verified Portal ↗
                </a>
              </div>
